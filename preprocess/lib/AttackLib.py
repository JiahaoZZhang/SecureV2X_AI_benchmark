...
def random_pos_offset(
    ...
    *,
    ...
    if np.random.rand(1) < config_yaml['attack_rate']:
        ...
        for key, val in attack_config['parameter'].items():
            # position X modification
            noise = attack_config['x_min'] + np.random.choice([-1,1],size=config_yaml['duration']
                                        )*np.random.rand(config_yaml['duration'])*np.array((val['x_max']-val['x_min']))
            
            
            print(np.dtype(key))
            df.loc[start:start+config_yaml['duration'],str(key)] = df.loc[start:start+config_yaml['duration'],str(key)] + noise
    ...
    if np.random.rand(1) < config_yaml['attack_rate']:
        ...
        for key, val in attack_config['parameter'].items():
            if attack_config['mode'] == 'random':
                # add a random constant noise modification
                noise = np.repeat(np.random.randint(val),config_yaml['duration'])
            elif attack_config['mode'] == 'fixed':
                # add a fixed constant noise modification
                noise = np.repeat(val,config_yaml['duration'])
            
            df.loc[start:start+config_yaml['duration'],key] = df.loc[start:start+config_yaml['duration'],key] + noise
